...
async function main() {
  ...
  const projects = await Promise.all([
    prisma.project.create({
      data: {
        ...
        technologies: JSON.stringify(['React', 'Node.js', 'MongoDB', 'Stripe', 'Tailwind CSS']),
        featured: true,
        order: 1
      }
    }),
    prisma.project.create({
      data: {
        ...
        technologies: JSON.stringify(['React', 'TypeScript', 'Socket.io', 'PostgreSQL', 'Prisma']),
        featured: true,
        order: 2
      }
    }),
  ...
  const experiences = await Promise.all([
    prisma.experience.create({
      data: {
        ...
        endDate: null,
        current: true,
        description: 'Leading frontend development for multiple web applications, mentoring junior developers, and implementing best practices for code quality and performance.',
        technologies: JSON.stringify(['React', 'TypeScript', 'Next.js', 'Tailwind CSS']),
        order: 1
      }
    }),
    prisma.experience.create({
  ...
  const about = await prisma.about.create({
    data: {
      title: 'About Me',
      content: 'I am a passionate full-stack developer with over 5 years of experience building modern web applications. I specialize in React, TypeScript, and Node.js, and I love creating user-friendly, scalable solutions that solve real-world problems.',
